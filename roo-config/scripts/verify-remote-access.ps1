...
$filesToCheck = @(
    ...
    "roo-modes/custom\scripts\deploy-local-endpoints.ps1",
...
$deployScriptPath = Join-Path -Path $projectRoot -ChildPath "roo-modes/custom\scripts\deploy-local-endpoints.ps1"
...
if (Test-Path -Path $deployScriptPath) {
    ...
    Add-ReportEntry -Category "Script de déploiement" -Status "ERREUR" -Details "Le script de déploiement n'existe pas: $deployScriptPath" -Recommendation "Créez le script de déploiement à partir du modèle dans 'roo-modes/custom/docs/implementation/script-deploy-local-endpoints.md'."
}
