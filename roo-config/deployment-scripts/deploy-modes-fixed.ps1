# Script de déploiement des modes simples/complex pour Roo (Version corrigée)
...
Write-ColorOutput "   (Version corrigée avec correction d'encodage)" "Cyan"
...
# Appliquer la correction d'encodage au fichier source
Write-ColorOutput "Application de la correction d'encodage au fichier source..." "Yellow"
$fixEncodingScript = Join-Path -Path $projectRoot -ChildPath "roo-config\encoding-scripts\fix-encoding-simple.ps1"
if (Test-Path -Path $fixEncodingScript) {
    & $fixEncodingScript -SourcePath $configFilePath -Force
} else {
    Write-ColorOutput "Avertissement: Le script de correction d'encodage n'a pas été trouvé." "Yellow"
}
...
try {
    ...
    $jsonContent = [System.IO.File]::ReadAllText($configFilePath)
    ...
    # Écrire le contenu en UTF-8 avec BOM pour une meilleure compatibilité avec Windows
    $utf8Encoding = New-Object System.Text.UTF8Encoding $true
    [System.IO.File]::WriteAllText($destinationFile, $jsonString, $utf8Encoding)
