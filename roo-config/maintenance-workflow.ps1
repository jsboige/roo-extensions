...
function Fix-EncodingIssues {
    ...
    Write-ColorOutput "3. Correction avancée (fix-encoding-advanced.ps1)" "White"
    Write-ColorOutput "4. Correction finale optimisée (fix-encoding-final.ps1)" "White"
    ...
    switch ($choice) {
        "1" {
            & ".\diagnostic-scripts\diagnostic-rapide-encodage.ps1" -FilePath $filePath -Fix
        }
        "2" {
            & ".\encoding-scripts\fix-encoding-complete.ps1" -SourcePath $filePath
        }
        "3" {
            & ".\encoding-scripts\fix-encoding-advanced.ps1" -SourcePath $filePath -CreateBackup $true
        }
        "4" {
            & ".\encoding-scripts\fix-encoding-final.ps1" -SourcePath $filePath
        }
        "5" {
...
function Diagnose-Configuration {
    ...
    Write-ColorOutput "1. Vérifier l'encodage d'un fichier (diagnostic-rapide-encodage.ps1)" "White"
    Write-ColorOutput "2. Vérifier les modes déployés (verify-deployed-modes.ps1)" "White"
    Write-ColorOutput "3. Diagnostic complet des problèmes d'encodage (encoding-diagnostic.ps1)" "White"
    ...
    switch ($choice) {
        "1" {
            $filePath = Read-Host "Entrez le chemin du fichier à analyser"
            if (Test-Path $filePath) {
                & ".\diagnostic-scripts\diagnostic-rapide-encodage.ps1" -FilePath $filePath -Verbose
            } else {
                Write-ColorOutput "Erreur: Le fichier $filePath n'existe pas." "Red"
            }
        }
        ...
        "3" {
            $filePath = Read-Host "Entrez le chemin du fichier à analyser"
            if (Test-Path $filePath) {
                $diagnosticScript = "diagnostic-scripts/encoding-diagnostic.ps1"
                if (Test-Path $diagnosticScript) {
                    & ".\$diagnosticScript" -FilePath $filePath
                } else {
                    Write-ColorOutput "Erreur: Le script de diagnostic n'existe pas à l'emplacement $diagnosticScript" "Red"
