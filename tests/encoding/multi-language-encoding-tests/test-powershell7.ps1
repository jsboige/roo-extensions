#!/usr/bin/env pwsh
# ==============================================================================
# Script: test-powershell7.ps1
# Description: Tests d'encodage pour PowerShell 7+ (Cross-platform)
# Auteur: Roo Debug Mode
# Date: 2025-10-29
# ==============================================================================

# Configuration UTF-8 explicite pour PowerShell 7+
...
function Get-DiagnosticInfo {
    ...
    return @{
        ...
        CodePage = if ($IsWindows) { &{chcp} 2>$null; Write-Output $LASTEXITCODE } else { "N/A (Unix)" }
...
function Test-ConsoleDisplay {
    ...
    Write-Host "Platforme: $(if ($diag.IsWindows) { 'Windows' } elseif ($diag.IsLinux) { 'Linux' } elseif ($diag.IsMacOS) { 'macOS' } else { 'Inconnue' })" -ForegroundColor Gray
...
function Test-ProcessTransmission {
    ...
    try {
        # Test avec pipe (PowerShell 7+ supporte mieux les pipes)
        $pipeResult = $Content | pwsh -Command "Write-Host '$Content'" 2>&1
        
        # Test avec redirection
        $tempFile = "results\pipe-test-$TestName.txt"
        $Content | pwsh -Command "Out-File -FilePath '$tempFile' -Encoding UTF8" 2>$null
...
function Test-EnvironmentVariables {
    ...
    try {
        ...
        [System.Environment]::SetEnvironmentVariable("TEST_EMOJI_PS7", $Content)
        
        # Lire la variable
        $envValue = [System.Environment]::GetEnvironmentVariable("TEST_EMOJI_PS7")
...
# Fonction de test des fonctionnalités PowerShell 7+
function Test-PowerShell7Features {
    ...
    try {
        # Tester les fonctionnalités spécifiques à PowerShell 7+
        $features = @{
            HasForEachObject = $null -ne (Get-Command ForEach-Object -ErrorAction SilentlyContinue)
            HasTernaryOperator = $true  # PowerShell 7+ supporte l'opérateur ternaire
            HasPipelineChainOperators = $true
            HasErrorActionCommonParameter = $true
            HasGetError = $true
            HasTryCatchFinally = $true
            HasNullCoalescing = $true
            HasNullConditional = $true
            HasSwitchExpression = $true
        }
        
        return @{ 
            Success = $true; 
            Diagnostic = $diag; 
            Result = "Fonctionnalités PowerShell 7+: $($features | ConvertTo-Json -Compress)" 
        }
    } catch {
        return @{ Success = $false; Diagnostic = $diag; Error = $_.Exception.Message }
    }
...
Write-Host "  TESTS D'ENCODAGE - POWERSHELL 7+ (CROSS-PLATFORM)" -ForegroundColor Cyan
Write-Host "═════════════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host ""

# Test 1: Caractères accentués simples
$accentedText = "é è à ù ç œ æ â ê î ô û"
$result = Test-ConsoleDisplay "PS7-Accented" $accentedText
$testResults += $result

$result = Test-FileWrite "PS7-Accented-File" $accentedText "accented-ps7.txt"
$testResults += $result

$result = Test-FileRead "PS7-Accented-Read" "sample-accented.txt"
$testResults += $result

# Test 2: Emojis simples
$simpleEmojis = "✅ ❌ ⚠️ ℹ️"
$result = Test-ConsoleDisplay "PS7-SimpleEmojis" $simpleEmojis
$testResults += $result

$result = Test-FileWrite "PS7-SimpleEmojis-File" $simpleEmojis "simple-emojis-ps7.txt"
$testResults += $result

$result = Test-FileRead "PS7-SimpleEmojis-Read" "sample-emojis.txt"
$testResults += $result

# Test 3: Emojis complexes
$complexEmojis = "🚀 💻 ⚙️ 🪲 📁 📄 📦 🔍 📊 📋 🔬 🎯 📈 💡 💾 🔄 🏗️ 📝 🔧 ✨"
$result = Test-ConsoleDisplay "PS7-ComplexEmojis" $complexEmojis
$testResults += $result

$result = Test-FileWrite "PS7-ComplexEmojis-File" $complexEmojis "complex-emojis-ps7.txt"
$testResults += $result

# Test 4: Transmission entre processus
$transmissionTest = "Test transmission: ✅ 🚀 💻"
$result = Test-ProcessTransmission "PS7-Transmission" $transmissionTest
$testResults += $result

# Test 5: Variables d'environnement
$envTest = "Variable env: ✅ 🚀 💻"
$result = Test-EnvironmentVariables "PS7-Environment" $envTest
$testResults += $result

# Test 6: Fonctionnalités PowerShell 7+
$result = Test-PowerShell7Features "PS7-Features"
$testResults += $result

# Test 7: Option système UTF-8 worldwide language support
...
try {
    if ($IsWindows) {
        ...
        $testResults += @{
            ...
            Diagnostic = Get-DiagnosticInfo "PS7-SystemSupport"
        ...
        $locale = if ($IsLinux) { locale } elseif ($IsMacOS) { locale } else { "N/A" }
        $testResults += @{
            Success = $true
            Diagnostic = Get-DiagnosticInfo "PS7-SystemSupport"
            Result = "Système Unix: $locale"
        }
    }
} catch {
    $testResults += @{
...
$resultsJson | Out-File -FilePath "results\powershell7-results.json" -Encoding UTF8

Write-Host ""
Write-Host "═════════════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host "  RÉSUMÉ DES TESTS POWERSHELL 7+" -ForegroundColor Cyan
Write-Host "═════════════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host ""
...
Write-Host ""
Write-Host "Résultats détaillés sauvegardés dans: results\powershell7-results.json" -ForegroundColor Cyan
Write-Host "Tests PowerShell 7+ terminés" -ForegroundColor Green