 ...
 * Ce script teste la nouvelle option extract_markdown_structure de l'outil list_directory_contents
 */
...
const TEST_DIR = path.join(__dirname, 'temp-quickfiles-markdown-test');
...
async function runTests() {
  console.log(`${COLORS.cyan}=== Test de l'extraction de structure markdown du MCP QuickFiles ===${COLORS.reset}`);
  ...
  try {
    ...
    // Tester la fonctionnalité d'extraction de structure markdown
    console.log(`\n${COLORS.blue}Test de la fonctionnalité d'extraction de structure markdown${COLORS.reset}`);
    
    // Simuler l'appel à l'outil list_directory_contents avec extract_markdown_structure=true
    const request = {
      jsonrpc: '2.0',
      id: 1,
      method: 'tools/call',
    ...
    if (response && !response.error) {
      ...
      const outputPath = path.join(TEST_DIR, 'markdown-output.md');
      ...
      if (content.includes('📁') && content.includes('📄')) {
        ...
        if (content.includes('.md')) {
          ...
          // Vérifier si les titres des fichiers markdown sont extraits avec les icônes appropriées
          if (content.includes('📑') || content.includes('📌')) {
            console.log(`${COLORS.green}✓ Les titres des fichiers markdown sont extraits avec les icônes appropriées${COLORS.reset}`);
            
            // Vérifier les titres spécifiques des fichiers markdown de test
            if (content.includes('Test Project') && content.includes('User Guide') && content.includes('API Reference')) {
              console.log(`${COLORS.green}✓ Les titres spécifiques des fichiers markdown sont correctement extraits${COLORS.reset}`);
            } else {
              console.log(`${COLORS.yellow}⚠ Certains titres de fichiers markdown semblent manquer${COLORS.reset}`);
            }
          } else {
            console.log(`${COLORS.red}✗ Les titres des fichiers markdown ne sont pas extraits correctement${COLORS.reset}`);
            console.log(`${COLORS.yellow}⚠ Vérifiez que le paramètre extract_markdown_structure est bien pris en compte${COLORS.reset}`);
          }
        } else {
