# Script d'organisation des fichiers par catégorie - Mission Nettoyage Git (Corrigé)
# Date: 2025-10-19
# Objectif: Analyser et catégoriser TOUS les fichiers modifiés (repo principal + sous-modules)

Write-Host "=== ORGANISATION COMPLÈTE DES FICHIERS PAR CATÉGORIE ===" -ForegroundColor Cyan
Write-Host "Date: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')" -ForegroundColor Gray

# Récupérer les fichiers du repo principal
$mainRepoFiles = git status --porcelain
Write-Host "Analyse du repo principal : $($mainRepoFiles.Count) fichiers..." -ForegroundColor Yellow

# Récupérer les fichiers du sous-module mcps/internal
$submoduleFiles = @()
try {
    Set-Location "mcps/internal"
    $submoduleStatus = git status --porcelain
    if ($submoduleStatus) {
        # Ajouter le préfixe du sous-module
        $submoduleFiles = $submoduleStatus | ForEach-Object { "M mcps/internal/$($_.Substring(3))" }
    }
    Set-Location "../../"
} catch {
    Set-Location "../../"
    Write-Host "Erreur lors de l'analyse du sous-module: $($_.Exception.Message)" -ForegroundColor Red
}

# Combiner tous les fichiers
$allFiles = $mainRepoFiles + $submoduleFiles
Write-Host "Total des fichiers à analyser : $($allFiles.Count)" -ForegroundColor Green
...
foreach ($category in $categories.Keys) {
    ...
    if ($files.Count -gt 0) {
        ...
        switch ($category) {
            ...
            "Submodules" { 
                $commitMessage = "chore(submodule): Nettoyage et synchronisation sous-module mcps/internal"
            }
            "Other" { 
...
Write-Host "Total des fichiers à traiter : $totalFiles" -ForegroundColor Cyan