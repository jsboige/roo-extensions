...
<#
.SYNOPSIS
    Test script for Invoke-RooStagedWorkflow wrapper
...
. "$RepoRoot\scripts\scheduling\Invoke-RooStagedWorkflow.ps1"
...
$latestLog = Get-ChildItem "$RepoRoot\outputs\scheduling\logs" -Filter "roo-staged-*.log" |
...
if ($latestLog) {
    ...
    Get-Content $latestLog.FullName | ForEach-Object { Write-Host "  $_" }
}
