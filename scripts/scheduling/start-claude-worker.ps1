    ...
    try {
        ...
        foreach ($PrRepo in @('jsboige/roo-extensions', 'jsboige/jsboige-mcp-servers')) {
            ...
            if ($LASTEXITCODE -eq 0 -and $ExistingPR) {
                ...
                # gauche). Fausse collision => issue skippee a tort. .NET regex, pas de
                # couche de quoting ici.
        ...
        "github" {
            if ($Task.issueNumber) {
                try {
                    ...
                    # #3752 F2: same no-leak guarantee as New-PullRequest.
                    try {
...
function New-WorkerPR {
    ...
    try {
        ...
        # PR title (max 70 chars). Double-quotes are stripped (#3752 F1): a "
        # interpolated into the cmd string of --title ""$Title"" would close
        # the cmd.exe quoting context — task subjects derive from issue
        # titles, which can carry quotes.
        $CleanSubject = ($Task.subject -replace '^\[TASK\]\s*', '' -replace '^\[URGENT\]\s*', '') -replace '"', ''
...
"@
        ...
        # #3752 F2: the temp body-file must not leak if the cmd invocation
        # throws — cleanup runs even on failure.
        $PrOutput = $null
        try {
