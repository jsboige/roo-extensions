...
| `claude/executor-preflight.ps1` | Pré-vol transactionnel executor : pull parent, matérialisation/vérification du gitlink `mcps/internal`, garantie d'un build RSM frais avant toute autre phase, puis détection non bloquante des erreurs API OAuth-expired récentes (#3169 option b — WARN visible, le cycle continue) |
...
| `claude/test-glm-context.ps1` | Teste la taille réelle de la fenêtre de contexte GLM via l'API configurÃ©e en envoyant des prompts de taille croissante avec un code secret, afin de détecter une troncature silencieuse |
| `claude/test-glm-markers.ps1` | Teste la troncature de contexte GLM par marqueurs — place 18 marqueurs uniques à intervalles d'environ 10K tokens sur ~180K tokens puis demande au modèle de les rapporter tous, afin de détecter une troncature silencieuse |
