...
12. **`.claude/` = SANCTUAIRE** (harnais uniquement). **JAMAIS** de fichiers temporaires, scratchpads, body-files pour `gh`, ou intermediaires de travail — meme supprimes apres usage. Si un outil requiert un body-file (ex `gh issue create --body-file`), utiliser `$TEMP` / `C:\Users\MYIA\AppData\Local\Temp\` OU passer le contenu inline via heredoc (`--body "$(cat <<'EOF' ... EOF)"`). Seule exception : `.claude/local/` (gitignored, deprecated mais tolere pour legacy). **Violation 2026-04-21** : `temp-issue-sk-agent.md` cree dans `.claude/` racine pour contourner un parsing heredoc — **ne doit pas se reproduire**, reflex par defaut = `$TEMP`.
