...
```
0. GROUNDING SDDD (codebase_search + roosync_search si disponible)
         |
...
### Grounding SDDD (Step 0)

Avant toute investigation, effectuer un bookend SDDD si les outils semantiques sont disponibles :

```bash
# Recherche semantique dans le code (si EMBEDDING_* configure)
codebase_search(query: "concept du bug en anglais", workspace: "CHEMIN_WORKSPACE")

# Recherche dans les taches Roo (si roo-state-manager disponible)
