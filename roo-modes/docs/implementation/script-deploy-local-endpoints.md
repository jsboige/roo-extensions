...
2. Créez un fichier `deploy-local-endpoints.ps1` dans le répertoire `custom-modes/scripts/`
...
Write-ColorOutput "custom-modes/docs/implementation/deploiement-autres-machines.md" "White"
...
```powershell
$scriptContent = Get-Content -Path "custom-modes/docs/implementation/script-deploy-local-endpoints.md" -Raw
$scriptContent -match '```powershell([\s\S]*?)```' | Out-Null
$script = $matches[1].Trim()
Set-Content -Path "custom-modes/scripts/deploy-local-endpoints.ps1" -Value $script
```
