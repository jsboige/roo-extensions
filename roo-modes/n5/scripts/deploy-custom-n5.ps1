# Script de déploiement de l'architecture à 5 niveaux avec préfixe custom-n5
# Ce script déploie les configurations des modes pour l'architecture à 5 niveaux
# avec des slugs préfixés par "custom-n5-" pour éviter les conflits avec les modes natifs

# Paramètres
param (
    [switch]$Force = $false,
    [switch]$SkipBackup = $false,
    [string]$ConfigFile = "$PSScriptRoot\..\configs\custom-n5-modes.json",
...
# Fonction principale
function Main {
    Write-ColorOutput "=== Déploiement de l'architecture à 5 niveaux avec préfixe custom-n5 ===" -ForegroundColor "Magenta"
