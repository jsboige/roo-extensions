...
foreach ($mcpName in $internalMcpConfigs.Keys) {
    if (-not $mcpSettings.mcpServers.ContainsKey($mcpName)) {
        ...
        Write-Host "$mcpName existe déjà, mise à jour..." -ForegroundColor Yellow
        $mcpSettings.mcpServers[$mcpName] = $internalMcpConfigs[$mcpName]
        $addedCount++
    }
}
...
try {
    ...
    Write-Host "Fichier mcp_settings.json mis à jour avec succès" -ForegroundColor Green
} catch {
    Write-Host "ERREUR: Impossible d'écrire le fichier mcp_settings.json" -ForegroundColor Red
...
Write-Host "MCPs internes ajoutés/mis à jour: $addedCount" -ForegroundColor Green
Write-Host "Total MCPs configurés: $($mcpSettings.mcpServers.Count)" -ForegroundColor Green
Write-Host ""
Write-Host "MCPs internes configurés:" -ForegroundColor Yellow
