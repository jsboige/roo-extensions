...
if ($foundPython) {
    try {
        ...
        Write-Host "❌ Erreur lors de la vérification/installation de markitdown_mcp: $errorMsg" -ForegroundColor Red
    }
}
...
try {
    ...
    $errorMsg = $_.Exception.Message
    Write-Host "❌ Erreur lors de l'installation de Playwright: $errorMsg" -ForegroundColor Red
}
...
foreach ($mcp in $internalMCPs) {
    ...
    if (Test-Path "$mcpPath/package.json") {
        ...
        try {
            ...
            $errorMsg = $_.Exception.Message
            Write-Host "❌ Erreur lors de l'installation des dépendances pour $mcp: $errorMsg" -ForegroundColor Red
        }
        ...
        if (Test-Path "$mcpPath/tsconfig.json") {
            try {
                ...
                $errorMsg = $_.Exception.Message
                Write-Host "❌ Erreur lors de la recompilation de $mcp: $errorMsg" -ForegroundColor Red
            }
        }
...
try {
    ...
    try {
        ...
        $errorMsg = $_.Exception.Message
        Write-Host "❌ Le serveur ne répond pas encore: $errorMsg" -ForegroundColor Red
    }
    
} catch {
    $errorMsg = $_.Exception.Message
    Write-Host "❌ Erreur lors du démarrage du serveur: $errorMsg" -ForegroundColor Red
}
