...
### RÈGLE CRITIQUE : Pas de fichiers temporaires dans le workspace

**INTERDI :** Créer des fichiers temporaires (query.graphql, query.json, etc.) dans le workspace racine pour les requêtes gh CLI.

**Pourquoi ?**

- Ces fichiers finissent dans le statut git comme non trackés
- Roo ne fait pas correctement le cleanup
- Les commandes échouent en boucle si les fichiers ne sont pas gérés

**CORRECT :** Passer les requêtes inline via heredoc ou stdin

```bash
# ✅ CORRECT - Requête inline avec heredoc
...
# ✅ CORRECT - Requête simple en une ligne
gh api graphql -f query='{ user(login: "jsboige") { projectV2(number: 67) { title } } }'
```
...
```bash
# ❌ INCORRECT - Fichier temporaire dans workspace
cat > query.graphql <<'EOF'
query { ... }
EOF
gh api graphql -f query=$(cat query.graphql)
# Le fichier query.graphql reste dans le workspace !
```

**Si fichiers temporaires absolument nécessaires :** Utiliser un répertoire dédié avec cleanup

```bash
# À éviter si possible
TEMP_DIR=.tmp/roo-temp
mkdir -p $TEMP_DIR
cat > $TEMP_DIR/query.graphql <<'EOF'
...
EOF
gh api graphql -f query=$(cat $TEMP_DIR/query.graphql)
rm -rf $TEMP_DIR
```
