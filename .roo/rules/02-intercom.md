...
**⚠️ DEPRECATED : Privilégier le dashboard RooSync (`roosync_dashboard`).**

Le fichier `.claude/local/INTERCOM-{MACHINE}.md` n'est qu'un fallback si le MCP échoue.

**Règles essentielles (si usage fallback) :**
- **TOUJOURS** ajouter à la FIN (ordre chronologique)
- **JAMAIS** insérer au début, supprimer ou modifier les messages existants
- Préférer `apply_diff` ou `Add-Content` plutôt que `write_to_file` (échec fréquent sur >500 lignes)

Exemple rapide (apply_diff) :
```
apply_diff(
  path: ".claude/local/INTERCOM-{MACHINE}.md",
  diff: "<<<<<<< SEARCH\n[dernier ---]\n======= REPLACE\n[dernier ---]\n\n## [DATE] sender → receiver [TYPE]\nContenu...\n\n---\n>>>>>>> REPLACE"
)
```
