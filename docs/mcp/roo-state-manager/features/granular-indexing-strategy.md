...
4.  **`code_citation` (Indexé, sélectif)**: Représente une citation de code (ex: extrait de référence depuis un fichier, un gist, ou une conversation antérieure). Actif depuis `feat(indexer) #959` (PR #959). Vectorisé seulement quand la heuristic `isCodeCitation` (ChunkExtractor.ts:158-179) le qualifie — intent-conservative par design ("false-negatives are cheap, false-positives cost signal"). Voir `code_citation_samples_c*.json` archive pour les mesures empiriques.
