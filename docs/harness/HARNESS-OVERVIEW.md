...
## 2. `roo-state-manager` — les 17 outils

C'est le serveur MCP central du harnais, écrit en TypeScript, vivant dans le sous-module `mcps/internal`. Il expose **17 outils** au handshake MCP.

> **Le compte est 17, pas 34.** Les consolidations « CONS » ont regroupé des dizaines d'outils unitaires en outils-familles paramétrés par une `action`. Le chiffre 34 circule encore dans des documents antérieurs à ce regroupement ; il est faux depuis. La mesure qui fait autorité est le handshake : ce qu'une session voit réellement.
...
**Anti-double-claim.** Avant de coder sur une issue, vérifier qu'aucune PR ouverte ne la référence (les **deux dépôts**, frontière de mot sur le numéro), puis vérifier et poser le verrou **sur l'issue elle-même** — `python scripts/github/check_issue_claim.py NNN` puis `--claim "intention"` — **avant** de modifier quoi que ce soit. Le `[CLAIMED]` dashboard reste la narration du cycle ; le **registre de verrous** est le commentaire d'issue, horodaté par le `createdAt` serveur (jamais un stamp de corps), et un `[DONE]`/`[RESULT]`/`--release` le lève à la livraison ([ADR 017](adr/017-issue-claim-locus.md), #3676). *Origine :* trois implémentations parallèles de la même issue, environ douze heures de travail dupliqué. Corollaire appris plus tard : **l'urgence produit la collision** — un relevé fait à un instant ne protège pas d'une PR créée une minute après. Et l'outage GDrive du 16/08 a montré le défaut du locus dashboard : quand il tombe, tout l'organe claim tombe avec lui.
...
**Une garde qui vérifie une propriété voisine de celle qui compte ne garde rien.** Quatre exemples de la même famille se sont produits sur le harnais. Aucun n'est de l'inattention : ce sont des vérifications sincères qui mesurent à côté.

*Un processus sain derrière un point de montage mort.* Un service de surveillance local rapporte « OK » parce que le binaire tourne, mais le volume réseau qu'il est censé relayer n'est plus joignable depuis des heures. Les écritures que le service juge « réussies » partent dans le vide — le rapport est sincère, le contenu n'arrive nulle part. La garde vérifiait la vivacité du processus ; la propriété qui comptait était la vivacité du montage. Les deux sont indépendantes, et le test ne touchait que la première.

*Une relance qui ne relance rien.* Un ordonnanceur décide de redémarrer un service qui ne répond plus. Mais l'instance jugée défaillante est déjà figée dans son démarrage : la nouvelle tentative laisse l'existant en place, et l'attente de grâce est plus courte que le temps d'initialisation réellement mesuré. Deux erreurs qui se masquaient mutuellement : un redémarrage qui ne redémarre pas, doublé d'un délai qui n'aurait pas suffi même si le redémarrage avait eu lieu. La garde vérifiait « une commande a été exécutée » ; la propriété qui comptait était « le service répond à nouveau ».

*Un test d'ancestralité employé pour juger la fraîcheur.* Une machine déclare « submodule à jour » parce que sa copie locale est un ancêtre de la référence distante. Le statut est vrai : la copie est bien un ancêtre. Le problème est qu'un commit plus ancien est *d'autant plus sûrement* un ancêtre qu'on est en retard — la garde passe d'autant mieux qu'on est plus loin de la cible. La propriété mesurée (non-orphelin) est voisine de la propriété voulue (à jour) mais ne la garantit pas. Une machine s'est déclarée fraîche avec deux commits de retard, et la garde au vert n'a même pas été surprise. La correction est triviale : *la fraîcheur est un test d'égalité*, pas d'ancestralité.

*Trois vérifications vertes sur les huit premiers caractères d'un identifiant dont trente-deux étaient faux.* Un enchaînement de contrôles — `git cat-file -e`, `merge-base --is-ancestor`, puis une relecture du gitlink comparée sur le seul préfixe — tolère les formes abrégées d'un identifiant de commit. Sur un préfixe de huit caractères, les trois gardes passent au vert. Le gitlink, lui, est un identifiant de quarante caractères dont seuls les huit premiers sont justes : les trente-deux autres portent un commit différent. La garde vérifiait la présence d'un commit qui *commençait* par le préfixe voulu ; la propriété qui comptait était l'égalité du commit complet. Une fois la relecture faite sur les quarante caractères, la divergence apparaît — mais aucune des trois gardes, seule, ne l'aurait vue. *Règle :* la valeur que l'on écrit doit être relue après écriture, et comparée sur sa longueur réelle, pas sur ce qui en passe à la vérification.

Le point commun de ces quatre cas n'est pas la complexité de la garde — trois d'entre elles sont d'une ligne. Il est dans l'écart entre la *propriété mesurée* et la *propriété qui compte*. Une mesure sincère n'est pas une mesure correcte : elle ne dit rien sur ce qui n'est pas dans son champ. La discipline qui aurait évité les quatre cas est la même à chaque fois : *avant d'ajouter une garde, nommer la propriété qui compte, puis vérifier que la garde la teste*.
...
1. **Maintenir un registre nom → handler.** L'enregistrement est explicite pour rendre auditable la liste des outils — c'est ce qui permet d'affirmer « 17 outils » sans grep dans tout `src/`.
...
**L'invariant.** `tools/list` répond toujours avec la liste complète des 17 outils, même dégradé. C'est ce qui permet à l'orchestrateur ou au coordinateur d'appeler un outil de diagnostic, d'apprendre que la capacité est dégradée, et d'agir — au lieu de recevoir un crash incompréhensible au démarrage.
