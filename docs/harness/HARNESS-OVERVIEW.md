...
*Trois vérifications vertes sur les huit premiers caractères d'un identifiant dont trente-deux étaient faux.* Un enchaînement de contrôles — `git cat-file -e`, `merge-base --is-ancestor`, `git ls-remote` — accepte toutes les formes abrégées d'un identifiant de commit. Sur un préfixe de huit caractères, les trois gardes passent au vert. Le gitlink, lui, est un identifiant de quarante caractères dont seuls les huit premiers sont justes : les trente-deux autres portent un commit différent. La garde vérifiait la présence d'un commit qui *commençait* par le préfixe voulu ; la propriété qui comptait était l'égalité du commit complet. Une fois la relecture faite sur les quarante caractères, la divergence apparaît — mais aucune des trois gardes, seule, ne l'aurait vue. *Règle :* la valeur que l'on écrit doit être relue après écriture, et comparée sur sa longueur réelle, pas sur ce qui en passe à la vérification.
