...
| **Quota GraphQL / instrument REST (#3623)** | Le REST `GET /rate_limit` rend **un bucket frais pour l'ensemble des ressources** (`graphql` ET `core`, `used=0` même après consommation avérée) — `reset` roulant = now+3600, **découplé** de la fenêtre ancrée GraphQL, sans recalage à la frontière. Le repli REST fonctionne mais **n'a pas d'indicateur de charge**. Contournements REST + `GH_TOKEN` explicite, routage par token ; harnais schedulés in-repo audités 17/09 : 8 scripts, 0 pinçage d'identité — fix = chantier scopé (hazard propagation `GH_TOKEN` → garde #3032) | `github-rate-limit-instrument.md` |
